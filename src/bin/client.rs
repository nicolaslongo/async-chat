...
async fn send_commands(mut to_server: net::TcpStream) -> ChatResult<()> {
    ...
    // command_lines.next() returns a STREAM ~ un iterador asíncrono.async_std
    // es una especie de híbrido entre un Iterator y un Future.
    // .next() es una especie de llamada a poll de este trait Stream
    while let Some(command_result) = command_lines.next().await {
        ...
        // Preparo efectivamente la request si salió todo bien y se la envio al server
        utils::send_as_json(&mut to_server, &request).await?;
        to_server.flush().await?;
    }
    // Si llegué hasta acá, salió todo de lujo el valor de mi Future después del último poll
    // será Ok
    Ok(())
}
...
fn main() -> ChatResult<()> {
    ...
    // wrap del comportamiento async en un bloque y se usa block_on para ejecutarlo
    task::block_on(async {
        ...
        // Esta llamada devuelve un Future que hace poll sobre from_server y to_server
        // pero cuando uno de ellos devuelve Ready(v) ese es el valor de nuestro resultado
        // y el otro se droppea
